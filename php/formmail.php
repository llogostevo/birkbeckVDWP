...
$DEF_ALERT = "stevens.lloyd@gmail.com";
...
$ATTACK_DETECTION_DUPS = array("fname", "email");
...
$SPECIAL_FIELDS = array(
	...
	"fname", // the real name of the person who filled in the form
	...
	if (!isset($s_cols) || empty($s_cols) || !is_string($s_cols)) {
		...
		if (!isset($s_cols) || empty($s_cols) || !is_string($s_cols)) {
			...
			/*
                 ...
                 */
			$a_column_list = array("email","fname");
			/*
                        ...
                        $s_to,$s_cc,$s_bcc,$a_raw_fields,$s_filter,$a_filter_list)
{
	...
	if (!$b_used_template) {
		...
		if (IsMailOptionSet("DupHeader")) {
			...
			if (!empty($SPECIAL_VALUES["email"])) {
				$res_hdr .= "From: " . MakeFromLine($SPECIAL_VALUES["email"],
				                                    $SPECIAL_VALUES["fname"]) . Settings::get('BODY_LF');
			}
		...
		//
		// put the fname and the email address at the top of the results
		// (if not excluded)
		//
		if (!IsMailExcluded("fname")) {
			array_unshift($a_fld_order,"fname");
			$a_clean_fields["fname"] = $SPECIAL_VALUES["fname"];
		}
...
function WriteLog($log_file)
{
	...
	$entry = $date . ":" . $SPECIAL_VALUES["email"] . "," .
	         $SPECIAL_VALUES["fname"] . "," . $SPECIAL_VALUES["subject"] . "\n";
	fwrite($log_fp,$entry);
	fclose($log_fp);
}
